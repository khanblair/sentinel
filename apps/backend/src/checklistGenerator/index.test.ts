...
describe("generateChecklistFromTestCase", () => {
  it("returns the steps and usage produced by the provider", async () => {
    ...
    const result = await generateChecklistFromTestCase({
      provider,
      urlPath: "/cart",
      steps: "1. add item\n2. apply code SAVE10",
    ...
    expect(result.steps).toEqual(["Navigate to /cart", "Apply code SAVE10", "Assert total reduced by 10%"]);
...
describe("generateChecklistFromInstruction", () => {
  it("passes the URL and free-text instruction through to the provider prompt", async () => {
    ...
    const result = await generateChecklistFromInstruction({
      provider,
